...
def setup_graph(graph, config):
    with graph.as_default():
        ...
        rnn_inputs = [tf.squeeze(i,squeeze_dims=[1]) for i in tf.split(x_one_hot, num_or_size_splits=config.num_steps, axis=1)]

        # Turn our y placeholder into a list of one-hot tensors
        y_one_hot = tf.one_hot(y, config.num_classes)
        y_as_list = [tf.squeeze(i, squeeze_dims=[1]) for i in tf.split(y_one_hot, num_or_size_splits=config.num_steps, axis=1)]
