...
# ### TODO
# 1. what is the meaning of weathersit temp atemp hum casual registered cnt?
# 
# on 2011-01-01 there are 16 people riding bike.
...
# cnt in 240 data from 2011-1-1 to 2011-1-9
# 
# but why there are only 10 days?
# 
# one day contains 10 data from morning to evening.
...
# regularize by mean and standard

# ### 将数据拆分为训练、测试和验证数据集
# 
# 我们将大约最后 21 天的数据保存为测试数据集，这些数据集会在训练完网络后使用。我们将使用该数据集进行预测，并与实际的骑行人数进行对比。

# How big is the data that we have for training?

# data?
# 
# 16872 rows 59 columns
...
# train:valid:test = 16875:1440:504
...
# ### mission
# 1. learn_rate
# 2. hidden notes quantaty.
# 3. epoch
# 
# model explanation
# 1. linear ==> sigmoid ==> linear ==> f(x) = x
# 
...
#             print("this is error, and final ", error.shape, final_outputs.shape)
            # error ==> 1 
            # y ==> 1
            ...
            # ==> h,1
#             print("hidden erro ", hidden_error.shape)
            # TODO: Backpropagated error terms - Replace these values with your calculations.
            hidden_error_term = hidden_error * hidden_outputs * (1 - hidden_outputs)
            # h,o==>h,1 
#             print("this is delta i h", delta_weights_i_h.shape, "X[:,None]", X[:None].shape, hidden_error_term.shape)
...
#             print(output_nodes * hidden_outputs)
#             print(delta_weights_h_o.shape)
    ...
    def run(self, features):
        ...
        final_inputs = np.dot(hidden_outputs, self.weights_hidden_to_output) # signals into final output layer
        final_outputs =  final_inputs# signals from final output layer 
#         print("{:.3f}".format(final_outputs))
#         print("weight", self.weights_input_to_hidden, self.weights_hidden_to_output)
...
# learning rate = 0.1 , reaches the local minimum of 0.2 ==> up learning rate to 0.5
# learning rate = 0.5, train 0.072, valid 0.167
...
# ### 回答
# 1. 预测效果并不好, 在前期的数据可以预测相对准确, 但是后期的数据差距比较大
# 2. 原因分析
#     - 模型没有偏执项bias, 范化能力不足.
#     - 模型的层数不足, 增加节点数无法提高对于feature的学习效果, 导致underfit.
# 
